window.JBForcePlateSessionStore = (() => {
  ...
  function normalizeAthlete(raw) {
    return {
      ...
      positionClass: String(raw?.positionClass || ''),
      shoots: String(raw?.shoots || raw?.shootingSide || raw?.shootsSide || raw?.dominantHand || raw?.handedness || ''),
      dateOfBirth: String(raw?.dateOfBirth || raw?.birthDate || raw?.dob || ''),
      heightCm: Number(raw?.heightCm ?? raw?.height ?? 0) || 0,
      sex: String(raw?.sex || raw?.gender || 'male'),
      photoId: String(raw?.photoId || raw?.photo || ''),
